...
from model.molcenter import MolCenter, make_cuda
from model.chemutils import get_mol
from model.vocab import Vocab, common_atom_vocab
from model.nnutils import create_pad_tensor, index_select_ND
from model.config import device

# 导入预训练任务头
from model.molecule_recovery_head import MoleculeRecoveryHead
from model.product_synthon_contrastive_head import ProductSynthonContrastiveHead
...
class G2RetroPDesignAlignedDataset(Dataset):
    ...
    def __init__(self, data_path, vocab_path, max_samples=None):
        ...
        print(f"分子词汇表大小: {len(self.vocab)} (设计方案要求：完整词汇表)")
        print(f"原子词汇表大小: {len(self.avocab)} (设计方案要求：common_atom_vocab)")
...
def g2retro_design_aligned_collate_fn(batch, vocab, avocab):
    ...
    try:
        ...
        batch_dict = {
            ...
            'pretrain_infos': pretrain_infos   # 预训练信息
        }
    ...
    def forward(self, batch, epoch=0):
        ...
        try:
            ...
            try:
                ...
                # 由于演示阶段可能缺少完整的标注数据，这里用占位符
                # 在实际应用中应该调用：self.reaction_center_head.predict_centers(...)
            ...
            print(f"  总损失 = {self.base_weight}×{losses['center'].item():.4f} + {self.recovery_weight}×{losses['recovery'].item():.4f} + {self.contrastive_weight}×{losses['contrastive'].item():.4f} = {total_loss.item():.4f}")
